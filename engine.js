(function(root,factory){
  ...
  const cloneState=s=>({current:{...s.current},mode:s.mode,output:s.output,initialOutput:s.initialOutput,suppressZeroPiece:s.suppressZeroPiece,doubleDoubleUsed:!!s.doubleDoubleUsed,doubleEchoUsed:!!s.doubleEchoUsed,echoActive:!!s.echoActive,echoOutput:s.echoOutput||0,echoPieceId:s.echoPieceId||null,zeroMemoryUsed:!!s.zeroMemoryUsed,lastNonZeroOp:s.lastNonZeroOp?{...s.lastNonZeroOp}:null,usedEdges:new Set(s.usedEdges),zeroCharges:cloneMap(s.zeroCharges),back:s.back.map(x=>({...x})),forward:s.forward.map(x=>({...x})),path:s.path.map(x=>({...x})),segments:s.segments.map(x=>({...x,from:{...x.from},to:{...x.to}})),events:s.events.map(x=>({...x})),traversals:s.traversals,rebounds:s.rebounds});
  function terminal(s,reason,meta={}){const mainOutput=s.output||0,echoOutput=s.echoActive?(s.echoOutput||0):0,output=mainOutput+echoOutput,echoEvents=s.echoActive?[{type:'double-echo-result',piece:s.echoPieceId,echoOutput,mainOutput,total:output}]:[],events=[...s.events,...echoEvents,{type:'die',reason}];return{output,mainOutput,echoOutput,gain:output-(s.initialOutput||0),path:s.path,segments:s.segments,events,zeroCharges:s.zeroCharges,reason,traversals:s.traversals,rebounds:s.rebounds,...meta}}
  ...
  function bestSignal(newPieceId,pieces,opts={}){
    ...
    function walk(s){
      ...
      const echoWasActive=s.echoActive;
      ...
      if(echoWasActive){const echoState={output:s.echoOutput};applyOp(outC.v,cur.double,echoState,false);s.echoOutput=echoState.output}
      s.events.push({type:'op',piece:cur.id,entryHalf:inC.half,exitHalf:outC.half,value:outC.v,op:op.type,before:op.before,after:op.after,add:op.add||0,factor:op.factor||0,delta:op.delta||0,doubleDouble:!!op.doubleDouble,reverse:s.mode===-1});
      if(outC.v!==0&&(op.type==='add'||op.type==='multiply'))s.lastNonZeroOp={piece:cur.id,value:outC.v,op:op.type,add:op.add||0,factor:op.factor||0};
      if(cur.id===opts.doubleEchoPieceId&&!s.doubleEchoUsed){s.doubleEchoUsed=true;s.echoActive=true;s.echoOutput=s.output;s.echoPieceId=cur.id;s.events.push({type:'double-echo-start',piece:cur.id,startOutput:s.echoOutput})}
      if(outC.v===0){
        const cap=cur.double?2:1,used=s.zeroCharges.get(cur.id)||0;
        if(s.suppressZeroPiece===cur.id){s.suppressZeroPiece=null;s.events.push({type:'zero-pass',piece:cur.id})}
        else if(used<cap){
          if(cur.id===opts.zeroMemoryPieceId&&!s.zeroMemoryUsed&&s.lastNonZeroOp){const mem=s.lastNonZeroOp,before=s.output;if(mem.op==='multiply')s.output*=mem.factor;else if(mem.op==='add')s.output+=mem.add;s.zeroMemoryUsed=true;s.events.push({type:'zero-memory',piece:cur.id,sourcePiece:mem.piece,op:mem.op,add:mem.add||0,factor:mem.factor||0,before,after:s.output})}
          s.zeroCharges.set(cur.id,used+1);s.mode*=-1;s.rebounds++;if(cur.double)s.suppressZeroPiece=cur.id;s.events.push({type:'rebound',piece:cur.id,charge:used+1});return walk(s)
