...
function createGame(E,opts={}){
  ...
  function assessContinuation(){
    const autoRerolls=[];s.needsReroll=false;if(s.cleared)return{autoRerolls};
    ...
    while(!hasLegal()&&((s.freeReroll||0)+(s.consumables?.reroll||0)>0)){
      const used=performReroll(true);if(!used)break;autoRerolls.push(used)
    }
    if(hasLegal()){s.blocked=false;s.failureReason=null;return{autoRerolls}}
  ...
  function finishPlacement(ctx){
    ...
    }else{const protection=ensureOpeningContinuation('draw');if(protection)s.events.push(protection);var continuation=assessContinuation()}
    s.events.push({type:'circuit-resonance',round:s.round+1,move:s.turn,...resonance});
    discoverCircuit(tile.id);
    s.running=false;return{ok:true,cleared:s.cleared,blocked:s.blocked,needsReroll:s.needsReroll,failureReason:s.failureReason,nextShopType:s.nextShopType,upgradeCoins,pendingCircuit:!!s.pendingCircuit,resonance,autoRerolls:continuation?.autoRerolls||[]}
  }
  ...
  function performReroll(automatic=false){
    if((s.freeReroll||0)+(s.consumables?.reroll||0)<=0)return null;
    ...
    const record={type:'reroll',round:s.round+1,roundTurn:s.roundTurn,source,automatic:!!automatic,remaining:s.consumables.reroll,freeRemaining:s.freeReroll||0,hand:s.hand.filter(Boolean).map(cloneTile)};
    s.events.push(record);if(protection)s.events.push(protection);
    return{source,automatic:!!automatic,remaining:s.consumables.reroll,freeRemaining:s.freeReroll||0,hand:record.hand}
  }
  function canUseReroll(){return !s.pendingCircuit&&((s.freeReroll||0)+(s.consumables?.reroll||0)>0)&&!s.running&&!s.cleared&&!s.shopOpen&&!(s.blocked&&s.failureReason==='no-legal-moves')&&s.failureReason!=='no-tiles'&&s.failureReason!=='placement-limit'}
  function reroll(){
    if(!canUseReroll())return{ok:false,reason:'state'};
    const used=performReroll(false),continuation=assessContinuation();
    return{ok:true,...used,autoRerolls:continuation?.autoRerolls||[],blocked:s.blocked,needsReroll:s.needsReroll,failureReason:s.failureReason}
  }
  ...
  function canUndo(){return!!s.undoFrame&&(s.consumables?.undo||0)>0&&!s.running&&!s.shopOpen&&!(s.blocked&&s.failureReason==='no-legal-moves')}
  ...
  function canOpenShop(){return !s.pendingCircuit&&!s.running&&!s.cleared&&!s.shopOpen&&!(s.blocked&&s.failureReason==='no-legal-moves')}
  function openShop(){
  ...
  function recoveryOptions(){
    const failure=s.failureReason,terminalNoLegal=failure==='no-legal-moves',shopAvailable=canOpenShop(),undo=canUndo();
  ...
  function restoreState(saved){
    ...
    if(!s.running&&!s.cleared&&!s.shopOpen&&(s.needsReroll||(s.blocked&&s.failureReason==='no-legal-moves'&&((s.freeReroll||0)+(s.consumables?.reroll||0)>0))))assessContinuation();
    return true
  }
  fresh(opts.seed);
