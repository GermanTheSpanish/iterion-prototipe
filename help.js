(function(root,factory){
  ...
  function rulebookSections(){
    const sections={
      ...
      persistence:{id:'persistence',displayName:'Machine Persistence',visual:'ROUND 1 MACHINE → ROUND 2 MACHINE',shortDescription:'Earlier placements shape later rounds.',rulesDescription:`Machine persistence is currently ${D.PERSIST_MACHINE_BETWEEN_ROUNDS?'ON':'OFF'}. Cleared rounds reset Output and round resources, while placed physical dominoes remain part of the machine. The board expands at the start of each new stage according to the configured board sizes. Market modifiers remain active for the rest of the run.`},
      economy:{id:'economy',displayName:'Shop / Market',visual:`SHOP anytime     MARKET every ${D.STAGE_SIZE} rounds`,shortDescription:'Shop handles survival; Market develops the build.',rulesDescription:`The Shop is available during active rounds and can also rescue some failed states. It sells random physical dominoes plus stored Move, Reroll and Undo tools. A random domino bought during a round enters the current hand if there is space, otherwise the reserve. The Market appears after each ${D.STAGE_SIZE}-round stage, shows up to ${D.MARKET_OFFER_COUNT||3} build-changing modifier offers, and allows at most ${D.MARKET_PURCHASE_LIMIT||1} purchase. Every purchase increases global Inflation by 1. Undo rewinds the last placement, but Shop purchases made after that placement stay spent and stay owned.`},
      modifiers:{id:'modifiers',displayName:'Modifiers',visual:'BASE TILE + MODIFIER',shortDescription:'Market modifiers create build-specific rules.',rulesDescription:`${modifierText()} Long Run checks for at least ${D.LONG_RUN_UNIQUE_THRESHOLD||10} unique routed physical dominoes. A physical star can pay at most once per Move, even when a route rebounds or an Echo follows it.`}
    };
  ...
  function tileRecord(state,tileId,tier){
    ...
    return Object.freeze({upgradeTier:tier,starCoins:tier,bestOutput,plays:turns.length,doubleDoubleActive:state.doubleDoubleTileId===tileId,doubleEchoActive:state.doubleEchoTileId===tileId,zeroMemoryActive:state.zeroMemoryTileId===tileId,longRunActive:(state.mods||[]).includes('long-run')})
  }
