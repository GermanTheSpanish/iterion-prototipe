(function(root,factory){
  ...
  function rulebookSections(){
    const sections={
      ...
      hand:{id:'hand',displayName:'Hand / Draw / Reroll',visual:`HAND ${D.HAND_SIZE} → PLACE → DRAW 1`,shortDescription:`The hand holds up to ${D.HAND_SIZE} dominoes.`,rulesDescription:'After a successful placement, one replacement tile is drawn from the remaining physical set. Each round grants one free full-hand Reroll. You may use it manually; if the current hand has no legal placement, MONOID consumes available Rerolls automatically, free first and then stored, until a legal hand appears or none remain. The free Reroll does not stack into the next round. When every physical tile in the current set has been placed, a new complete POWER set is created automatically.'},
      ...
      economy:{id:'economy',displayName:'Shop / Market',visual:`SHOP anytime     MARKET every ${D.STAGE_SIZE} rounds`,shortDescription:'Shop handles survival; Market develops the build.',rulesDescription:`The Shop is available during active rounds and can rescue some failed states. It sells random physical dominoes plus stored Move, Reroll and Undo tools. Every round grants one free Reroll. When no legal placement remains, available Rerolls are consumed automatically, free first and then stored; if none remain, the round ends immediately. The free Reroll refreshes to one rather than accumulating when the next round begins. The Market appears after each ${D.STAGE_SIZE}-round stage and presents up to ${D.MARKET_OFFER_COUNT||3} valid build-mod offers. Choose at most one; you may continue without buying. Every purchase increases global Inflation by 1. In Endless, every placed domino adds 1 System Strain to every Shop and Market price. Undo removes the Strain from the placement it rewinds, while Shop purchases made after that placement stay spent and stay owned. LONG CHAIN can grant its full multi-star payout on only ${D.ENDLESS_LONG_RUN_ACTIVATIONS||7} qualifying Endless Moves; afterwards the normal highest-star-wins rule returns.`},
