(function(root,factory){
  ...
  function tileLocation(state,tileId,piece){
    if(piece)return'board';
    if((state.hand||[]).some(t=>t?.id===tileId))return'hand';
    if((state.reserve||[]).some(t=>t?.id===tileId))return'reserve';
    return'owned'
  }
  ...
  function inspectTile(state,tileId){
    ...
    const piece=(state.pieces||[]).find(p=>p.tile?.id===tileId)||null;
    const connections=piece?E.connectionsForPiece(piece,state.pieces||[]):[];
    const neighbourIds=[...new Set(connections.map(c=>c.toPieceId))];
    const modifiers=tileModifiers(state,tile);
    return Object.freeze({
      ...
      currentMachineState:Object.freeze({
        location:tileLocation(state,tileId,piece),
        placed:!!piece,
        pieceId:piece?.id??null,
        rotation:piece?.rr??null,
        axis:piece?.axis??null,
        upgradeTier:tile.upgrade||0,
        anchor:state.anchorId===tile.id,
        doubleDoubleActive:state.doubleDoubleTileId===tile.id,
        connectedPieceIds:Object.freeze(neighbourIds),
