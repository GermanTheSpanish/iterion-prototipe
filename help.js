(function(root,factory){
  ...
  function rulebookSections(){
    const sections={
      ...
      power:{id:'power',displayName:'POWER Sets',visual:`SET I ×1 → EXHAUST → SET II ×${D.POWER_SET_MULTIPLIER||2}`,shortDescription:'Exhausting a physical set unlocks a stronger complete set.',rulesDescription:`Set I is the normal double-six set. When every physical tile in the current set has been placed, NOMON automatically adds another complete 28-tile set with new physical IDs. Set II and all later automatic sets are POWER ×${D.POWER_SET_MULTIPLIER||2}: 1/3/5 use doubled multiplier factors, 2/4/6 use doubled additions, and 0 is unchanged. POWER never changes matching values, parity, placement legality or routing. Random dominoes bought after POWER is unlocked inherit the current POWER generation. Buying extra tiles before exhaustion delays the next set. Circuit Tiles still turn black; a ×${D.POWER_SET_MULTIPLIER||2} mark preserves their POWER identity.`},
      economy:{id:'economy',displayName:'Shop / Market',visual:`SHOP anytime     MARKET every ${D.STAGE_SIZE} rounds`,shortDescription:'Shop handles survival; Market develops the build.',rulesDescription:`The Shop is available during active rounds and can rescue some failed states. It sells random physical dominoes plus stored Move, Reroll and Undo tools. Every round grants one free Reroll. It is spent before stored Rerolls and refreshes to one rather than accumulating when the next round begins. The Market appears after each ${D.STAGE_SIZE}-round stage and presents up to ${D.MARKET_OFFER_COUNT||3} valid build-mod offers. Choose at most one; you may continue without buying. Every purchase increases global Inflation by 1. Undo rewinds the last placement, but Shop purchases made after that placement stay spent and stay owned.`},
