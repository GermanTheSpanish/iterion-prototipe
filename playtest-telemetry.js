(function(root,factory){
  ...
  function create(options={}){
    ...
    function scoreText(value){
      const n=Number(value);if(!Number.isFinite(n))return value==null?'-':String(value);
      const a=Math.abs(n);return a>=1e12?n.toExponential(3).replace('e+','e'):String(Math.round(n))
    }
    function placementLine(p){
      const best=p.evaluationComplete&&p.bestLegalOutput!=null?p.bestLegalOutput:'-',ratio=p.chosenVsBestRatio==null?'-':Number(p.chosenVsBestRatio).toFixed(4),topology=p.topologyAfter||p.topologyBefore||{},bestTile=p.bestPlacement?.tileId?`${p.bestPlacement.tileId}@${p.bestPlacement.x},${p.bestPlacement.y},r${p.bestPlacement.rr}`:'-',coverage=p.clearCoverage==null?'-':`${(Number(p.clearCoverage)*100).toFixed(1)}%(${p.clearCoverageClearCount??0}/${p.clearCoverageSampleCount??0} ${p.clearCoverageComplete?'exact':'sample'})`,dist=p.outputDistribution||{},ctx=p.context||{},deck=p.deckAfter||p.deckBefore||{};
      return`T${p.turn??'?'} R${p.round}/S${p.stage}: chosen=${p.chosenOutput??'-'} best=${best} ratio=${ratio} clear=${coverage} dist=p50:${scoreText(dist.median)},p90:${scoreText(dist.p90)},max:${scoreText(dist.max)} legal=${p.legalPlacementCount??'-'} evaluated=${p.evaluatedPlacementCount??0}/${p.legalPlacementCount??0} complete=${p.evaluationComplete?'yes':'no'} evalMs=${Math.round(finite(p.evaluationMs))} bestPlacement=${bestTile} rebounds=${p.context?.rebounds??0} splits=${p.context?.splits??0} reentry=${ctx.reentryOperationCount??0}/${ctx.operationCount??0} retrace=${ctx.retraceMoveCount??0} zeroReturn=${ctx.zeroReturnCount??0} visited=${ctx.uniqueVisitedPieceCount??0}/${topology.machineSize??'-'} circuits+${p.context?.newCircuits??0} deck=g${deck.generation??'-'},remaining:${deck.remainingTiles??'-'},hand:${deck.handCount??'-'},reserve:${deck.reserveCount??'-'} topology=tiles:${topology.machineSize??'-'},cycles:${topology.cycleRank??'-'},T:${topology.tJunctionCount??'-'},X:${topology.crossCount??'-'},doubles:${topology.doubleCount??'-'},zeros:${topology.zeroCount??'-'},Zleaf:${topology.zeroLeafCount??'-'},maxDegree:${topology.maxDegree??'-'}`
    }
    function text(){
