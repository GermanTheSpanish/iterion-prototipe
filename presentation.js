(function(root,factory){
  ...
  function debugFilename(text){text=brandDebugText(text);const version=(text.match(/^MONOID DEBUG v([^\r\n]+)/m)||[])[1]||'unknown',runId=(text.match(/^Run ID:\s*([^\r\n]+)/m)||[])[1]||'run',safe=s=>String(s).trim().replace(/[^a-zA-Z0-9._-]+/g,'-');return`${BRAND}_DEBUG_v${safe(version)}_${safe(runId)}.txt`}
  function installUiPolish(root){
    ...
    const score=doc.getElementById('score'),scoreDetail=doc.getElementById('scoreDetail'),targetDetail=doc.getElementById('targetDetail'),scoreNote=doc.getElementById('scoreNote'),board=doc.getElementById('board'),runlog=doc.getElementById('runlog'),menuCopy=doc.getElementById('copyrun'),viewRun=doc.getElementById('viewrun'),gameMenu=doc.getElementById('gameMenu'),toast=doc.getElementById('toast');if(!score||!scoreDetail||!targetDetail||!board)return;
    ...
    // branch values remain visible on the board and engine state stays the authority.
    new MutationObserver(()=>{const live=[...board.querySelectorAll('.signalValue')].filter(el=>Number.isFinite(Number(el.dataset.output)));if(live.length===1)tweenScore(Number(live[0].dataset.output))}).observe(board,{childList:true,subtree:true});
    ...
    const shareDebug=async textOverride=>{const text=brandDebugText(typeof textOverride==='string'?textOverride:debugTextFromUi());if(!text){feedback('Debug unavailable');return false}const filename=debugFilename(text),file=new File([text],filename,{type:'text/plain'});if(root.navigator?.share){let canFiles=true;try{if(root.navigator.canShare)canFiles=root.navigator.canShare({files:[file]})}catch(_){canFiles=false}if(canFiles){try{await root.navigator.share({files:[file],title:`${BRAND} DEBUG`});feedback('DEBUG READY');return true}catch(err){if(err?.name==='AbortError')return false}}}try{downloadDebug(text,filename);feedback('DEBUG READY');return true}catch(_){feedback('Debug export failed');return false}};
    const patchRunlog=()=>{const ta=runlog?.querySelector('.runDataText');if(ta)ta.value=brandDebugText(ta.value);const button=runlog?.querySelector('.tool');if(!button||button.dataset.debugShare==='1')return;button.dataset.debugShare='1';button.textContent='SHARE .TXT';button.onclick=()=>shareDebug(runlog.querySelector('.runDataText')?.value||'')};if(runlog){new MutationObserver(patchRunlog).observe(runlog,{childList:true,subtree:true});patchRunlog()}if(menuCopy){menuCopy.textContent='Share debug .txt';menuCopy.onclick=async()=>{try{if(gameMenu?.open)gameMenu.close()}catch(_){}await shareDebug()}}root.NomonUiPolish=Object.freeze({shareDebug,debugTextFromUi,paintProgress,tweenScore})
  }
  return Object.freeze({compact,exact,scoreDisplay,cascadeDelay,effectLifetime,operationHalf,fitFontSize,signalPlan,forkBlock,progressState,brandDebugText,debugFilename,installUiPolish,CASCADE,BRAND});
});