(function(root,factory){
  ...
  const CASCADE=Object.freeze({introMs:Object.freeze([600,600,560,520,480,440]),tailMs:320,minMs:60,decay:0.70,maxLabels:8,retainedLabels:10,settleMs:300,resolveHoldMs:190,resolveFadeMs:180,finalMs:500,scoreTweenMs:360,subtotalHoldMs:360,settleItemMs:360,resonanceSettleMs:420,contributionLimit:8});
  ...
  function cascadeOperationKey(lane,piece,half,operation){return [String(lane||''),String(piece??''),String(half??''),String(operation||'')].join('|')}
  function cascadeRepeatTier(count){count=Math.max(1,Math.floor(Number(count)||1));return count>=5?5:count>=3?3:count>=2?2:1}
  ...
  function progressState(score,target){score=Math.max(0,Number(score)||0);target=Math.max(1,Number(target)||1);if(score<target)return{stage:'target',progress:Math.max(0,Math.min(1,score/target)),next:'TARGET'};if(score<target*3)return{stage:'clear',progress:Math.max(0,Math.min(1,(score-target)/(target*2))),next:'×3'};if(score<target*5)return{stage:'star1',progress:Math.max(0,Math.min(1,(score-target*3)/(target*2))),next:'×5'};if(score<target*10)return{stage:'star2',progress:Math.max(0,Math.min(1,(score-target*5)/(target*5))),next:'×10'};return{stage:'star3',progress:1,next:'MAX'}}
  ...
  function installUiPolish(root){
    ...
    const style=doc.createElement('style');style.id='nomon-ui-polish';style.textContent=`
      ...
      .signalValue.cascadeOperation{overflow:visible!important}
      .signalValue.cascadeRepeat2>span{background:#e7e6e1!important;border-color:#c5c3bc!important}
      .signalValue.cascadeRepeat3>span{background:#c7c6c0!important;border-color:#a6a49d!important}
      .signalValue.cascadeRepeat5>span{background:#4c4b47!important;border-color:#4c4b47!important;color:#fff!important}
      .signalValue[data-lane*="."].cascadeRepeat2{background:#e7e6e1!important}
      .signalValue[data-lane*="."].cascadeRepeat3{background:#c7c6c0!important}
      .signalValue[data-lane*="."].cascadeRepeat5{background:#4c4b47!important;border-color:#4c4b47!important;color:#fff!important}
      .signalValue[data-lane*="."].cascadeRepeat5 small,.signalValue[data-lane*="."].cascadeRepeat5 span{color:#fff!important}
      .cascadeRepeatBadge{position:absolute!important;right:-6px!important;top:-7px!important;display:grid!important;place-items:center!important;min-width:15px!important;height:15px!important;padding:0 3px!important;border:1px solid rgba(21,21,21,.28)!important;border-radius:8px!important;background:#fff!important;color:#20201d!important;font:800 8px/1 ui-monospace,SFMono-Regular,Menlo,monospace!important;letter-spacing:-.04em!important;box-shadow:none!important}
      .cascadeRepeat5 .cascadeRepeatBadge{background:#20201d!important;color:#fff!important;border-color:#20201d!important}
      .cascadeRepeatBadge.cascadeRepeatBump{animation:cascadeRepeatBump 140ms ease-out both!important}
      .signalValue.cascadeOperation.cascadeToScore{animation:cascadeOperationOut 360ms ease-in both!important}
      ...
      @keyframes cascadeOperationOut{from{opacity:1;transform:translate(-50%,-50%) scale(1)}to{opacity:0;transform:translate(-50%,-58%) scale(.92)}}
      @keyframes cascadeRepeatBump{0%{transform:scale(.72)}55%{transform:scale(1.12)}100%{transform:scale(1)}}
  ...
  return Object.freeze({compact,exact,scoreDisplay,cascadeDelay,effectLifetime,operationHalf,cascadeOperationKey,cascadeRepeatTier,fitFontSize,signalPlan,forkBlock,armLabel,terminalContributions,groupCascadeContributions,cascadeSettlementPlan,progressState,tileViewModel,longChainViewModel,hudViewModel,brandDebugText,debugFilename,installUiPolish,CASCADE,BRAND});
});
