(function(root,factory){
  ...
  const CASCADE=Object.freeze({introMs:Object.freeze([600,600,560,520,480,440]),tailMs:320,minMs:60,decay:0.70,maxLabels:8,retainedLabels:10,settleMs:300,resolveHoldMs:190,resolveFadeMs:180,finalMs:500,scoreTweenMs:520,subtotalHoldMs:720,settleItemMs:620,resonanceSettleMs:760,operationFlashMs:380,structuralFxMs:1520,contributionLimit:8});
  ...
  function multiplierText(value){value=Math.max(0,Number(value)||0);return value<1000?scaledText(value):compact(value)}
  function scoreDisplay(score,target){score=Math.max(0,Number(score)||0);target=Math.max(1,Number(target)||1);const ratio=score/target,multiplier=multiplierText(ratio),overdrive=ratio>=1000,text=overdrive?`×${multiplier}`:compact(score),note=score<target?(score>0&&ratio>=.95?`${compact(target-score)} to target`:'Last move'):overdrive?'TARGET MULTIPLIER':ratio>=2?`×${multiplier} target`:'Target reached';return{score:text,target:compact(target),note,ratio,multiplier,overdrive,mode:overdrive?'multiplier':'absolute'}}
  ...
  function installUiPolish(root){
    ...
    const style=doc.createElement('style');style.id='nomon-ui-polish';style.textContent=`
      ...
      .opfx.operationFlash{z-index:42!important;font-size:22px!important;font-weight:850!important;line-height:1!important;letter-spacing:-.035em!important;white-space:nowrap!important;animation:cascadeOperationFlash var(--cascade-flash-ms,380ms) ease-out both!important}
      .opfx.operationFlash.add{color:#fff!important;-webkit-text-stroke:1.45px #151515!important}
      .opfx.operationFlash.multiply{color:#151515!important;-webkit-text-stroke:2.25px #fff!important}
      ...
      .cascadeSubtotal{z-index:44!important;display:grid!important;grid-template-columns:auto!important;gap:3px!important;min-width:64px!important;padding:7px 9px!important;border:1px solid rgba(21,21,21,.28)!important;border-radius:4px!important;background:rgba(251,250,246,.97)!important;color:#20201d!important;-webkit-text-stroke:0!important;box-shadow:none!important;text-align:left!important;animation:cascadeChipIn 120ms ease-out both!important}
      .cascadeSubtotal.echoContribution{border-style:dashed!important}.cascadeSubtotal small{font-size:10px!important;font-weight:750!important;line-height:1!important;letter-spacing:.06em!important;color:#6f6b64!important}.cascadeSubtotal strong{font-size:22px!important;font-weight:850!important;line-height:1!important;font-variant-numeric:tabular-nums!important}
      ...
      @keyframes cascadeOperationFlash{0%{opacity:0;transform:translate(-50%,-50%) scale(.78)}14%,82%{opacity:1;transform:translate(-50%,-50%) scale(1)}100%{opacity:0;transform:translate(-50%,-64%) scale(.96)}}
