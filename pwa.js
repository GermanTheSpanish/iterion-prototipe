(function(root){
  ...
  const GUARD_KEY='monoidBackGuardV1',SHELL_BUILD='20260926.1';
  ...
  function loadUiLayer(){
    ...
    if(!doc.querySelector('script[data-monoid-mod-collection]')){const script=doc.createElement('script');script.src=`mod-collection.js?v=${SHELL_BUILD}`;script.async=false;script.dataset.monoidModCollection='true';doc.body.appendChild(script)}
    if(!doc.querySelector('script[data-monoid-board-camera]')){const script=doc.createElement('script');script.src=`board-camera.js?v=${SHELL_BUILD}`;script.async=false;script.dataset.monoidBoardCamera='true';doc.body.appendChild(script)}
  }
