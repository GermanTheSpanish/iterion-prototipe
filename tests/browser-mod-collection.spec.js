...
test('Game Selection opens one-screen 4 × 7 Mod Collection',async({page},testInfo)=>{
  ...
  const access=page.locator('#modCollectionSelectionButton');await expect(access).toBeVisible();await expect(access).toHaveText('MODS · 10/28');
  await access.click();const dialog=page.locator('#modCollectionDialog');await expect(dialog).toBeVisible();
  await expect(dialog.locator('.modCollectionCell')).toHaveCount(28);await expect(dialog.locator('.modCollectionCell.isUnlocked')).toHaveCount(10);await expect(dialog.locator('.modCollectionCell.isLocked')).toHaveCount(18);
...
test('Run Menu uses the same collection without mutating the saved run',async({page})=>{
  ...
  const before=await page.evaluate(()=>localStorage.getItem('iterion.activeRun.v1'));await page.locator('#menuButton').click();const access=page.locator('#modCollectionMenuButton');await expect(access).toHaveText('MODS · 10/28');await access.click();
