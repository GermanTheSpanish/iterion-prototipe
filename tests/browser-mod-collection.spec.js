...
test('Game Selection opens one-screen 4 × 7 Mod Collection',async({page},testInfo)=>{
  ...
  const access=page.locator('#modCollectionSelectionButton');await expect(access).toBeVisible();await expect(access).toHaveText('MODS · 24/28');
  await access.click();const dialog=page.locator('#modCollectionDialog');await expect(dialog).toBeVisible();
  await expect(dialog.locator('.modCollectionCell')).toHaveCount(28);await expect(dialog.locator('.modCollectionCell.isUnlocked')).toHaveCount(24);await expect(dialog.locator('.modCollectionCell.isLocked')).toHaveCount(4);
  ...
  await expect(dialog.locator('[data-domino="1|3"] .collectionModCode')).toHaveText('FN');await expect(dialog.locator('[data-domino="1|4"] .collectionModCode')).toHaveText('FM');await expect(dialog.locator('[data-domino="1|6"] .collectionModCode')).toHaveText('TW');
  await expect(dialog.locator('[data-domino="2|4"] .collectionModCode')).toHaveText('SQ');await expect(dialog.locator('[data-domino="2|5"] .collectionModCode')).toHaveText('C6');
  await expect(dialog.locator('[data-domino="3|4"] .collectionModCode')).toHaveText('CW');await expect(dialog.locator('[data-domino="3|5"] .collectionModCode')).toHaveText('FT');await expect(dialog.locator('[data-domino="3|6"] .collectionModCode')).toHaveText('RL');
  await expect(dialog.locator('[data-domino="4|5"] .collectionModCode')).toHaveText('CP');await expect(dialog.locator('[data-domino="4|6"] .collectionModCode')).toHaveText('RS');await expect(dialog.locator('[data-domino="5|5"] .collectionModCode')).toHaveText('FG');
...
test('Run Menu uses the same collection without mutating the saved run',async({page})=>{
  ...
  const before=await page.evaluate(()=>localStorage.getItem('iterion.activeRun.v1'));await page.locator('#menuButton').click();const access=page.locator('#modCollectionMenuButton');await expect(access).toHaveText('MODS · 24/28');await access.click();
...
test('future Mod discovery flips one reserved domino and persists meta progress',async({page})=>{
  ...
  const result=await page.evaluate(()=>{window.IterionMods.register({id:'qa-future-mod',displayName:'QA FUTURE MOD',kind:'market-tile-mod',collectionTile:[0,2],collectionCode:'QA',collectionDefaultUnlocked:false,shortDescription:'Collection integration fixture.'});window.MonoidModCollection.render();return window.MonoidModCollection.unlock('qa-future-mod')});
  expect(result).toEqual({ok:true,domino:'0|2'});const tile=page.locator('[data-domino="0|2"]');await expect(tile).toHaveClass(/isUnlocked/);await expect(tile).toHaveClass(/justUnlocked/);await expect(tile.locator('.collectionModCode')).toHaveText('QA');await expect(page.locator('.modCollectionProgress strong')).toHaveText('25/28');expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('monoid.modCollection.v1')))).toContain('qa-future-mod')
});
