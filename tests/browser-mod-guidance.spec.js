...
test('Modifier primer teaches BUILD → REWARD without asking players to memorise the catalogue',async({page})=>{
  ...
  await expect(page.locator('.modifierTutorTitle')).toHaveText('MODS LIVE ON TILES');await expect(page.locator('.modifierTutorScene .domino')).toHaveCount(2);await expect(page.locator('.modifierTutorSceneState')).toContainText('INSTALLED');await expect(page.locator('.modifierTutorNote')).toContainText('Hold any Modded tile');
  const type=await page.evaluate(()=>({body:parseFloat(getComputedStyle(document.querySelector('.modifierTutorBody')).fontSize),note:parseFloat(getComputedStyle(document.querySelector('.modifierTutorNote')).fontSize),visual:document.querySelector('.modifierTutorVisual').getBoundingClientRect().height,installedAnimation:getComputedStyle(document.querySelector('.modifierTutorTerminal .focus')).animationName}));expect(type.body).toBeGreaterThanOrEqual(18);expect(type.note).toBeGreaterThanOrEqual(14);expect(type.visual).toBeGreaterThan(200);expect(type.installedAnimation).toContain('modifierInstalledPulse');
  await page.locator('.modifierNext').click();await expect(page.locator('.modifierTutorTitle')).toHaveText('BUILD → REWARD');await expect(page.locator('.modifierTutorScene .domino')).toHaveCount(3);await expect(page.locator('.modifierTutorSceneState')).toContainText('BUILD');await expect(page.locator('.modifierTutorSceneState')).toContainText('ACTIVE ×3');await expect(page.locator('.modifierTutorBody')).toContainText('Build the condition');await expect(page.locator('.modifierTutorNote')).toContainText('CORNER');await expect(page.locator('.modifierTutorNote')).toContainText('×3');expect(await page.locator('.modifierTutorCorner .north').evaluate(el=>getComputedStyle(el).animationName)).toContain('modifierBuildNeighbour');
  await page.locator('.modifierNext').click();await expect(page.locator('.modifierTutorTitle')).toHaveText('SOME MODS CHANGE SIGNALS');await expect(page.locator('.modifierTutorScene .domino')).toHaveCount(2);await expect(page.locator('.modifierTutorSceneState')).toContainText('TELEPORT');expect(await page.locator('.modifierTutorPortArrow').evaluate(el=>getComputedStyle(el).animationName)).toContain('modifierPortTravel');
  await page.locator('.modifierNext').click();await expect(page.locator('.modifierTutorTitle')).toHaveText('SOME MODS CHANGE THE MACHINE');await expect(page.locator('.modifierTutorScene .domino')).toHaveCount(4);await expect(page.locator('.modifierTutorSceneState')).toContainText('10+ UNIQUE TILES');expect(await page.locator('.modifierTutorChain .modifierTutorTile').first().evaluate(el=>getComputedStyle(el).animationName)).toContain('modifierRoutePulse');
});
