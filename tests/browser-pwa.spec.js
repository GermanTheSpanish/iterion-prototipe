...
test('title reveal follows DOMINO order, build is visible and tutorials are grouped',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>window.__MONOID_BUILD)).toBe('20260915.1');
  await expect(page.locator('#devBuildStamp')).toContainText('v0.31.1 · build 20260915.1');
  ...
  await page.locator('#titleCard').click();await expect(page.locator('#tutorialHubButton')).toBeVisible();await expect(page.locator('#replayTutorial')).toBeHidden();await expect(page.locator('#systemsTutorial')).toBeHidden();
  await page.locator('#tutorialHubButton').click();await expect(page.locator('#tutorialHub')).toBeVisible();await expect(page.locator('#tutorialHub')).toContainText('BASICS · 2 MIN');await expect(page.locator('#tutorialHub')).toContainText('GAME MECHANICS · 3 MIN');await expect(page.locator('#tutorialHub')).toContainText('MODIFIERS · 2 MIN')
});

test('modifier mini tutorial covers DD, DE, ZM and Long Chain',async({page})=>{
  await page.setViewportSize({width:390,height:844});await page.goto('http://127.0.0.1:4173/');await page.locator('#titleCard').click();await page.locator('#tutorialHubButton').click();await page.locator('[data-tutorial="modifiers"]').click();
