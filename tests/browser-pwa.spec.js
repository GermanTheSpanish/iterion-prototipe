...
const BUILD='20260923.1',NEXT_BUILD='20260923.2';
...
test('title reveal follows DOMINO order without a pre-animation flash',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>window.__MONOID_BUILD)).toBe('20260923.1');
  await expect(page.locator('#devBuildStamp')).toContainText(`v0.43.0 · build ${BUILD}`);
...
test('silent update detection offers reload and preserves the active run before refresh',async({page})=>{
  await page.addInitScript(nextBuild=>{
    ...
    window.fetch=(input,init)=>{
      ...
      if(url.includes('build.json'))return Promise.resolve(new Response(JSON.stringify({version:'0.43.0',build:nextBuild}),{status:200,headers:{'Content-Type':'application/json'}}));
