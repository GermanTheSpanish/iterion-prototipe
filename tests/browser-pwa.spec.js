...
const BUILD='20260924.4',NEXT_BUILD='20260924.5';
...
test('title reveal follows DOMINO order without a pre-animation flash',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>window.__MONOID_BUILD)).toBe('20260924.4');
  await expect(page.locator('#devBuildStamp')).toContainText(`v0.44.2 · build ${BUILD}`);
...
test('silent update detection offers reload and preserves the active run before refresh',async({page})=>{
  await page.addInitScript(nextBuild=>{
    ...
    window.fetch=(input,init)=>{
      ...
      if(url.includes('build.json'))return Promise.resolve(new Response(JSON.stringify({version:'0.44.2',build:nextBuild}),{status:200,headers:{'Content-Type':'application/json'}}));
