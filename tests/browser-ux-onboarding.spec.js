...
test('Shop and Market occupy the board surface and keep the machine intact',async({page})=>{
  ...
  await page.locator('#helpButton').click();await page.locator('#overlayPrimary').click();await expect(page.locator('#overlayTitle')).toHaveText('MARKET');await expect(page.locator('.marketOffer')).toHaveCount(3);await assertCommerceOnBoard(page);await expect(page.locator('.shopFoot')).toContainText('Buy one mod, or leave it.');await assertNoPageScroll(page)
});
