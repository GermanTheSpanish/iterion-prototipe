...
test('BASICS keeps placements canonical while teaching Zero, rebound and T-Split without hidden coordinates',async({page})=>{
  ...
  await expect(page.locator('#overlayTitle')).toHaveText('TILE SHOP');expect(await page.evaluate(()=>window.__monoidGame.state().pieces.length)).toBe(7);await assertCommerceIsDedicatedOverlay(page);await expect(page.locator('#nextModifiersFromBasics')).toBeVisible();await expect(page.locator('#nextModifiersFromBasics')).toHaveText('NEXT · MODIFIERS')
});
