...
test('v028 live SCORE progress, abbreviations and debug file sharing',async({page},testInfo)=>{
  ...
  await page.addInitScript(()=>{
    ...
    Object.defineProperty(navigator,'share',{configurable:true,value:async data=>{const f=data.files[0];window.__sharedDebug={name:f.name,type:f.type,text:await f.text()}}});
  });
...
test('v028 POWER reads as pale material and Stars use the divider',async({page},testInfo)=>{
  ...
  await page.evaluate(()=>{
    const d=document.querySelector('#hand .domino');d.classList.add('powerTile','power2');const p=document.createElement('i');p.className='powerMark';p.textContent='×2';d.appendChild(p);const u=document.createElement('i');u.className='upgradeDot u2';d.appendChild(u);
  });
  const tile=page.locator('#hand .domino').first();
  expect(await tile.evaluate(el=>getComputedStyle(el,'::before').backgroundColor)).toBe('rgb(219, 230, 236)');
