...
test('v028 live SCORE progress, abbreviations and debug file sharing',async({page},testInfo)=>{
  ...
  await expect(page).toHaveTitle('MONOID v0.38.0');
  ...
  await expect.poll(()=>page.evaluate(()=>window.__sharedDebug?.name||'')).toMatch(/^MONOID_DEBUG_v0\.38\.0_.+\.txt$/);
  const shared=await page.evaluate(()=>window.__sharedDebug);expect(shared.type).toBe('text/plain');expect(shared.title).toBe('MONOID DEBUG');expect(shared.text).toContain('MONOID DEBUG v0.38.0');expect(shared.text).toContain('Run ID:');expect(shared.text).toContain('PERFORMANCE TELEMETRY');
...
test('v028 debug export falls back to a downloadable txt file',async({page})=>{
  ...
  expect(download.suggestedFilename()).toMatch(/^MONOID_DEBUG_v0\.38\.0_.+\.txt$/);
});
