...
test('v028 resolved SCORE progress, abbreviations and debug file sharing',async({page},testInfo)=>{
  ...
  await expect(page).toHaveTitle('MONOID v0.47.0');
  ...
  await expect.poll(()=>page.evaluate(()=>window.__sharedDebug?.name||'')).toMatch(/^MONOID_PLAYTEST_v0\.46\.0_.+\.txt$/);
  const shared=await page.evaluate(()=>window.__sharedDebug);expect(shared.type).toBe('text/plain');expect(shared.title).toBe('MONOID PLAYTEST');expect(shared.text).toContain('MONOID PLAYTEST BATCH v1');expect(shared.text).toContain('MONOID DEBUG v0.47.0');expect(shared.text).toContain('Run ID:');expect(shared.text).toContain('PERFORMANCE TELEMETRY');expect(shared.text).toContain('Batch ID:');
...
test('v028 debug export falls back to a downloadable txt file',async({page})=>{
  ...
  expect(download.suggestedFilename()).toMatch(/^MONOID_PLAYTEST_v0\.46\.0_.+\.txt$/);
});
