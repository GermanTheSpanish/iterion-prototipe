...
test('game mode carousel keeps its frame and exposes Classic as the only playable progression',async({page})=>{
  ...
  await expect(page.locator('#modeName')).toHaveText('LOCKED');await expect(page.locator('#modeDescription')).toHaveText('Not available');await expect(page.locator('#startRun')).toBeDisabled();
...
test('critical raised title and screen isolation survive a missing presentation stylesheet',async({page})=>{
  ...
  expect(assets.every(url=>url.includes('?v=entry-0311')||url.endsWith('?v=20260926.4'))).toBe(true);
  await page.mouse.click(12,12);
  await expect(page.locator('#titleCard')).toBeHidden();
  await expect(page.locator('#gameSelection')).toBeVisible();
  await expect(page.locator('.app')).toBeHidden();
