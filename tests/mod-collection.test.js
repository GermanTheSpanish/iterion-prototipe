...
assert.equal(assigned.length,10,'all ten currently available Mods must be represented');
assert.equal(new Set(assigned.map(slot=>slot.mod.id)).size,10,'one Mod must occupy exactly one collection domino');
assert.equal(new Set(assigned.map(slot=>slot.mod.collectionCode)).size,10,'revealed collection codes must be unique');
