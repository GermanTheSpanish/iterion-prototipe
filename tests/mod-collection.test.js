...
assert.equal(assigned.length,24,'all twenty-four currently available Mods must be represented');
assert.equal(new Set(assigned.map(slot=>slot.mod.id)).size,24,'one Mod must occupy exactly one collection domino');
assert.equal(new Set(assigned.map(slot=>slot.mod.collectionCode)).size,24,'revealed collection codes must be unique');
