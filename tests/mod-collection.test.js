...
assert.equal(assigned.length,28,'all twenty-eight currently available Mods must be represented');
assert.equal(new Set(assigned.map(slot=>slot.mod.id)).size,28,'one Mod must occupy exactly one collection domino');
assert.equal(new Set(assigned.map(slot=>slot.mod.collectionCode)).size,28,'revealed collection codes must be unique');
...
assert.equal(slots.find(slot=>slot.key==='0|2').mod.id,'foundation');
assert.equal(slots.find(slot=>slot.key==='0|3').mod.id,'pair');
assert.equal(slots.find(slot=>slot.key==='0|4').mod.id,'bridge');
assert.equal(slots.find(slot=>slot.key==='0|5').mod.id,'gate');
assert.equal(slots.find(slot=>slot.key==='1|3').mod.id,'fan');
assert.equal(slots.find(slot=>slot.key==='1|4').mod.id,'frame');
assert.equal(slots.find(slot=>slot.key==='1|5').mod.id,'knot');
assert.equal(slots.find(slot=>slot.key==='1|6').mod.id,'twin');
assert.equal(slots.find(slot=>slot.key==='2|4').mod.id,'diode');
assert.equal(slots.find(slot=>slot.key==='2|5').mod.id,'return');
assert.equal(slots.find(slot=>slot.key==='2|6').mod.id,'mirror');
assert.equal(slots.find(slot=>slot.key==='3|3').mod.id,'triple-double');
assert.equal(slots.find(slot=>slot.key==='3|4').mod.id,'crown');
assert.equal(slots.find(slot=>slot.key==='3|5').mod.id,'frontier');
assert.equal(slots.find(slot=>slot.key==='3|6').mod.id,'merge');
assert.equal(slots.find(slot=>slot.key==='4|4').mod.id,'overload');
assert.equal(slots.find(slot=>slot.key==='4|5').mod.id,'hinge');
assert.equal(slots.find(slot=>slot.key==='4|6').mod.id,'resonator');
assert.equal(slots.find(slot=>slot.key==='5|5').mod.id,'forge');
assert.equal(slots.find(slot=>slot.key==='5|6').mod.id,'mint');
