...
assert.equal(assigned.length,20,'all twenty currently available Mods must be represented');
assert.equal(new Set(assigned.map(slot=>slot.mod.id)).size,20,'one Mod must occupy exactly one collection domino');
assert.equal(new Set(assigned.map(slot=>slot.mod.collectionCode)).size,20,'revealed collection codes must be unique');
