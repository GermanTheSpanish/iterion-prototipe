...
function testMarketOffersThreeAndLocksAfterOnePurchase(){
  ...
  const second=game.buyMarketMod(offers[1]);
...
function longRunIncome(enabled,duplicateStarEvent=false){
  E.setBoardSize(30,40);
  const game=Game.createGame(E,{seed:9,STARTING_COINS:0,TARGETS:Array(15).fill(1e15)});
  ...
  if(duplicateStarEvent)events.push({type:'op',piece:s.pieces[2].id,op:'multiply',factor:1,before:10,after:10,reverse:true});
  s.running=true;s.roundTurn=1;s.turn=1;s.score=0;s.cleared=false;s.blocked=false;s.failureReason=null;
  const result=game.finishPlacement({ok:true,tile:played.tile,p:played,trigger:2,sim:{output:10,events,reason:'test',rebounds:duplicateStarEvent?1:0,search:{starts:1,leaves:1,expanded:events.length}}});
  return{result,s};
}

function testLongRunPaysEveryUniqueStarOnce(){
  const normal=longRunIncome(false),long=longRunIncome(true),rebound=longRunIncome(true,true);
  ...
  assert.strictEqual(rebound.result.upgradeCoins,6,'revisiting a starred physical tile must not repay it');
...
testDoubleEchoAndZeroMemoryEngineEffects();
testLongRunPaysEveryUniqueStarOnce();
