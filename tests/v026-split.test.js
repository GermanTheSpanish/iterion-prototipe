...
const E=require('../engine.js'),G=require('../game.js'),C=require('../circuits.js'),D=require('../data.js'),V=require('../presentation.js');
let checks=0;
function check(name,fn){fn();checks++;console.log(`Split: ${name}`)}
function pieces(specs){return specs.map(([a,b,x,y,rr],i)=>{const p=E.pieceFrom({a,b},x,y,0,rr,i+1);p.tile={a,b,id:`physical-${i}`};return p})}
function fixture(left=5,right=4,d=3){return pieces([[d,d,6,8,0],[left,d,2,8,0],[d,right,10,8,0],[d,2,7,10,1]])}
function run(ps,options={}){return E.bestSignal(ps.length,ps,{initialOutput:5,bifurcate:true,...options})}
const events=(r,type)=>r.events.filter(e=>e.type===type);
E.setBoardSize(30,40);
check('complete two-arm T copies the full post-operation Score',()=>{
  const r=run(fixture());assert.equal(r.output,94);assert.equal(r.traversals,3);
  assert.deepEqual(events(r,'signal-start').map(e=>e.output),[15,15]);
