...
function consumeSupply(g){const s=g.state();s.placedTileIds=s.set.map(t=>t.id);s.hand=Array(5).fill(null);s.reserve=[];s.cleared=false;s.roundTurn=0;s.turn=Math.max(1,s.turn);g.assessContinuation()}
...
check('purchase before a POWER placement stays owned after its Undo',()=>{
  const g=lastTileGame();g.finishPlacement(g.beginPlacement(0,{x:10,y:8,rr:0}));g.openShop();const r=g.buyShopRandomTile();g.closeShop();assert(g.hasLegal());const s=g.state(),before=clone(s);let ctx;for(let i=0;i<5&&!ctx;i++){const c=g.candidatesForIndex(i)[0];if(c)ctx=g.beginPlacement(i,c)}assert(ctx?.ok);g.finishPlacement(ctx);assert(g.useUndo().ok);assert.deepEqual(g.state().set.find(t=>t.id===r.tile.id),r.tile);assert.equal(g.state().coins,before.coins);assert.equal(g.state().inflation,before.inflation);uniqueLocations(g.state());
});
