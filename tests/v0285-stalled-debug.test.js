...
assert.equal(g.recoveryOptions().recoverable,true,'no-legal-moves remains recoverable when the player can explicitly buy a Reroll');
assert.equal(g.recoveryOptions().ownedReroll,false);
assert.equal(g.recoveryOptions().toolReroll,true,'no-legal-moves must expose an affordable explicit Reroll purchase');
assert.equal(g.recoveryOptions().rerollRescue,true);
...
assert.match(debug,/Recovery: recoverable=yes .* ownedReroll=no .* buyReroll=yes@3c/);
