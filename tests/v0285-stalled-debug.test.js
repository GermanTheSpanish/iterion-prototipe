...
assert.equal(g.recoveryOptions().toolReroll,false,'terminal no-legal-moves must not offer post-failure Reroll purchases');
...
assert.match(debug,/Recovery: recoverable=no .* ownedReroll=no .* buyReroll=no/);
