...
{
  const{game,s}=completedBase(integratedOptions);
  assert.strictEqual(s.gameMode,'classic');
  assert.strictEqual(s.scoringModel,undefined,'Classic progression uses canonical scoring');
  assert.deepStrictEqual(game.boardSizeForStage(5),[30,40],'entering Endless does not grow the board immediately');
  ...
  assert.strictEqual(game.closeMarket(),true);
  assert.strictEqual(game.advance(),true);
  assert.strictEqual(s.round,15,'Classic progresses into R16 Endless');
  assert.deepStrictEqual(E.getBoardSize(),{G:30,H:40});
  assert.strictEqual(s.hand.length,5,'Endless keeps the normal five-tile Hand before Infinite');
...

{
  ...
  assert.strictEqual(game.advance(),true,'R60 clear advances into Infinite at R61');
  assert.strictEqual(s.round,60);
  assert.strictEqual(game.infinitePhase(),true);
  assert.strictEqual(game.handSizeForRound(),3);
  assert.strictEqual(s.hand.length,3,'Infinite creates a real three-tile Hand');
  ...
  const restored=Game.createGame(E,{...integratedOptions,seed:3702});
  assert.strictEqual(restored.restoreState(legacy),true);
  assert.strictEqual(restored.state().hand.length,3,'restoring an older five-tile Infinite save normalizes to three tiles');
...
{
  const{game}=completedBase(integratedOptions),legacy=game.exportState();
  legacy.state.gameMode='infinite-endless';legacy.state.scoringModel='deferred-v1';legacy.state.scoringFormula='legacy';
  legacy.state.endlessMode=true;legacy.state.round=60;
  const restored=Game.createGame(E,{...integratedOptions,seed:3703});
  assert.strictEqual(restored.restoreState(legacy),true);
  assert.strictEqual(restored.state().gameMode,'classic','legacy Infinite Endless saves migrate into the integrated Classic progression');
  assert.strictEqual(restored.state().scoringModel,undefined);
  assert.strictEqual(restored.state().scoringFormula,undefined);
  assert.strictEqual(restored.infinitePhase(),true,'legacy saves retain their current phase after migration');
}

{
  const{game}=completedBase(integratedOptions),legacy=game.exportState();
  legacy.state.gameMode='prototype';legacy.state.endlessMode=true;legacy.state.round=15;
  const restored=Game.createGame(E,{...integratedOptions,seed:3704});
  assert.strictEqual(restored.restoreState(legacy),true);
  assert.strictEqual(restored.state().gameMode,'classic');
  assert.strictEqual(restored.snapshot().endless.phase,'endless');
}

console.log('Classic → Endless → Infinite progression regressions passed');
