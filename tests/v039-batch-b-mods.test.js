...
function replay(pieces,modId,pieceId,value=2,initial=0){
  ...
  const result=E.replaySelectedScoring(base,initial,{pieces,modIdsByPiece:new Map([[pieceId,new Set([modId])]]),bridgeMultiplier:D.BRIDGE_MOD_MULTIPLIER,gateMultiplier:D.GATE_MOD_MULTIPLIER,fanMultiplier:D.FAN_MOD_MULTIPLIER,crownMultiplier:D.CROWN_MOD_MULTIPLIER});
  assert.deepEqual(result.path,base.path,'retained Batch B Mods must not rewrite selected routing');
  assert.deepEqual(result.segments,base.segments,'retained Batch B Mods must not rewrite route segments');
  return result
}
...
}
{
  ...
  s.cornerTileId='d2-3';fakeMarket(g,'gate');const info=g.marketOfferInfo('gate');
  assert(info.targetTiles.every(t=>t.a!==t.b),'Gate Market assignment must exclude doubles');assert(!info.targetTiles.some(t=>t.id==='d2-3'),'Gate must still respect one-Mod-per-tile');assert(info.targetTiles.some(t=>t.id==='d0-5'));
...
{
  const g=G.createGame(E,{seed:3902}),s=setPlaced(g,['d0-5','d1-3','d3-4']);s.gateTileId='d0-5';s.fanTileId='d1-3';s.crownTileId='d3-4';
  ...
  assert.equal(rs.gateTileId,'d0-5');assert.equal(rs.fanTileId,'d1-3');assert.equal(rs.crownTileId,'d3-4');
  assert.deepEqual(snap.tileMods.gate,['d0-5']);assert.deepEqual(snap.tileMods.fan,['d1-3']);assert.deepEqual(snap.tileMods.crown,['d3-4']);
  assert.match(restored.debugText(),/GT=d0-5 · FN=d1-3 · BO=- · CW=d3-4/);
}
