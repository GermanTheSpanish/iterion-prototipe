...
function replay(pieces,modId,pieceId,{value=2,knotCycles=0}={}){
  ...
  const result=E.replaySelectedScoring(base,10,{pieces,modIdsByPiece:new Map([[pieceId,new Set([modId])]]),knotCycleCountByPiece:new Map([[pieceId,knotCycles]]),knotMultiplier:D.KNOT_MOD_MULTIPLIER,mirrorMultiplier:D.MIRROR_MOD_MULTIPLIER});
  assert.deepEqual(result.path,base.path,'Batch D must not rewrite selected routing');assert.deepEqual(result.segments,base.segments,'Batch D must not rewrite selected route segments');return result
}
...
assert.deepEqual([D.FOUNDATION_MARKETS,D.FOUNDATION_COINS,D.KNOT_MOD_MULTIPLIER,D.MIRROR_MOD_MULTIPLIER,D.MINT_COINS],[3,3,4,3,1]);

{
  const p=piece(2,4,6,8,0,1),r=replay([p],'foundation',1);assert.equal(r.events[0].foundation,true);assert.equal(r.events[0].modMultiplier,1);assert.equal(r.output,12,'FOUNDATION no longer changes Score');
}
{
...
{
  const g=G.createGame(E,{seed:4101,STARTING_COINS:100}),s=setPlaced(g,['d0-2','d1-5','d2-6','d5-6']);s.cornerTileId='d1-5';s.marketCount=2;
  ...
  fakeMarket(g,'mirror');const info=g.marketOfferInfo('mirror');assert(info.targetTiles.every(t=>t.a!==t.b));assert(!info.targetTiles.some(t=>t.id==='d1-5'),'one-Mod-per-tile remains enforced');
}
{
...
{
  const g=G.createGame(E,{seed:4103}),s=setPlaced(g,['d0-2','d2-6','d5-6']);s.foundationTileId='d0-2';s.mirrorTileId='d2-6';s.mintTileId='d5-6';s.marketCount=5;s.foundationAssignedMarket=2;s.foundationLastPayoutMarket=4;s.mintPaidRound=0;
  ...
  assert.equal(rs.foundationTileId,'d0-2');assert.equal(rs.mirrorTileId,'d2-6');assert.equal(rs.mintTileId,'d5-6');assert.equal(rs.foundationLastPayoutMarket,4);
  assert.deepEqual(snap.tileMods.foundation,['d0-2']);assert.equal(snap.tileModState.foundationProgress,1);
  assert.deepEqual(P.tileViewModel(rs.set.find(t=>t.id==='d2-6'),rs).modifiers.map(m=>m.label),['MR']);assert.equal(H.inspectTile(rs,'d0-2').currentMachineState.foundationProgress,1);assert.equal(H.inspectTile(rs,'d5-6').currentMachineState.mintAvailable,false);
}
{
  const g=G.createGame(E,{seed:4104,STARTING_COINS:100}),s=setPlaced(g,['d0-2']);s.foundationTileId='d0-2';s.foundationAssignedMarket=0;s.foundationLastPayoutMarket=0;s.marketCount=2;s.cleared=true;s.intermissionResolved=false;s.nextShopType='market';
  assert(g.openIntermission());assert.equal(s.marketCount,3);assert.equal(s.coins,103);assert.equal(s.foundationLastPayoutMarket,3);assert.equal(s.events.filter(e=>e.type==='foundation-coins').length,1);
}
...
console.log('v0.47.0 retained Batch D and Foundation economy regressions passed');
