(function(){
  ...
  function renderInspector(){
    ...
    if(state.tollTileId===auxOverlay.tileId){
      ...
      overlaySecondary.onclick=()=>{const r=GAME.setTollArmed(!GAME.state().tollArmed);if(!r.ok){toast('Toll unavailable');return}persistGame();renderAuxOverlay();toast(r.armed?'TOLL ARMED · FIRST ACTIVATION −1c':'TOLL DISARMED')}
    }
  }
  ...
  function showClear(){
    ...
    const next=s.nextShopType;overlayPrimary.textContent=next==='market'?'MARKET':endless?'NEXT ENDLESS ROUND':'NEXT ROUND';overlayPrimary.onclick=()=>{if(next==='none'){advanceRound();return}const beforeRound=GAME.state().round,eventCursor=GAME.state().events.length;if(GAME.openIntermission()){const state=GAME.state(),marketClosed=state.events.slice(eventCursor).some(e=>e.type==='shop-close'&&e.shop==='market'&&e.reason==='insufficient-coins');if(marketClosed&&!state.shopOpen&&state.round>beforeRound){syncPlaytestContext();persistGame();hideOverlay();handFx.fill('normal');render();armDecisionTiming();toast(`MARKET CLOSED · INSUFFICIENT COINS · ROUND ${state.round+1}`);return}PT?.openMarket({offers:[...state.shopOffers]});persistGame();render()}else toast('Unavailable')};
  ...
  function showShop(){
    ...
    const random=$('shopRandomBuy');random.disabled=s.coins<randomCost;random.onclick=()=>{const r=GAME.buyShopRandomTile();if(!r.ok){toast(r.reason==='no-tiles'?'No random next-set tiles remain':'Not enough coins');return}shopRevealTile=r.tile;persistGame();toast(r.shopClosedReason==='insufficient-coins'?`[${r.tile.a}|${r.tile.b}] → ${r.delivery} · TILE SHOP CLOSED · INSUFFICIENT COINS`:`[${r.tile.a}|${r.tile.b}]${r.tile.powerMultiplier>1?` ×${r.tile.powerMultiplier}`:''} → ${r.delivery} · Inflation ${r.inflation}`);render()};
    overlayBody.querySelectorAll('[data-shop-tile-offer]').forEach(b=>{b.onclick=()=>{const r=GAME.buyShopTileOffer(b.dataset.shopTileOffer);if(!r.ok){toast(r.reason==='coins'?'Not enough coins':'Tile unavailable');return}persistGame();toast(r.shopClosedReason==='insufficient-coins'?`[${r.tile.a}|${r.tile.b}] → ${r.delivery} · TILE SHOP CLOSED · INSUFFICIENT COINS`:`[${r.tile.a}|${r.tile.b}] ×${r.tile.powerMultiplier||1} → ${r.delivery} · Inflation ${r.inflation}`);render()}});
    if(tutorial?.step===5){
