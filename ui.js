(function(){
  ...
  function showShop(){
    resetOverlay();const s=GAME.state(),x=GAME.snapshot(),randomCost=GAME.shopRandomPrice(),offers=M.all().filter(m=>m.kind==='consumable'),tileOffers=x.shop?.tileOffers||[],offerGeneration=x.shop?.tileOfferGeneration||Math.max(2,(s.setGeneration||1)+1),offerPower=tileOffers[0]?.tile?.powerMultiplier||1,strain=s.systemStrain||0;overlayTitle.textContent='SHOP';modalEl.classList.add('commerceModal');
