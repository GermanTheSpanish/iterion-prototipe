(function(){
  ...
  function showMarket(){
    ...
    const labels={'double-double':'DD','double-echo':'DE','zero-port':'ZP','parity-exchange':'PX','corner':'CR','long-line':'LN','overload':'OV','terminal':'TE'},assignments=[];
  ...
  function render(){
    ...
    shopBtn.innerHTML=`Tiles<small>${compact(s.coins)} coins</small>`;shopBtn.disabled=!!tutorial||uiBusy||!GAME.canOpenShop();const canBuyMove=GAME.canBuyTool('move'),canBuyReroll=GAME.canBuyTool('reroll'),canBuyUndo=GAME.canBuyTool('undo');moveBtn.innerHTML=`Move<small>${s.consumables.move?`+1 · ${s.consumables.move}`:'0 · BUY'}</small>`;moveBtn.disabled=!!tutorial||uiBusy||!(GAME.canUseMove()||canBuyMove);const rerollLabel=s.freeReroll?`Reroll · FREE${s.consumables.reroll?` + ${s.consumables.reroll}`:''}`:`Reroll · ${s.consumables.reroll||'0 · BUY'}`;rerollBtn.innerHTML=`Reroll<small>${rerollLabel.slice(9)}</small>`;rerollBtn.setAttribute('aria-label',rerollLabel);rerollBtn.disabled=!!tutorial||uiBusy||!(GAME.canUseReroll()||canBuyReroll);undoBtn.innerHTML=`Undo<small>${s.consumables.undo||'0 · BUY'}</small>`;undoBtn.disabled=!!tutorial||uiBusy||!(GAME.canUndo()||canBuyUndo);menuButton.disabled=!!tutorial||s.running||uiBusy;$('leaveTutorial').disabled=!!tutorial&&(uiBusy||s.running);
