(function(){
  ...
  function showMarket(){
    resetOverlay();const s=GAME.state(),x=GAME.snapshot(),nextStage=x.stage.index+1,nextSize=D.BOARD_SIZES[Math.min(nextStage-1,D.BOARD_SIZES.length-1)],randomCost=GAME.marketRandomPrice(),exactCost=GAME.marketExactPrice();
    ...
    overlayBody.innerHTML=`<div class="bigShop"><div class="shopHero"><div><div class="label">Stage ${x.stage.index} complete</div><strong>${s.coins}c</strong><div class="shopInflation">Inflation ${s.inflation}</div></div><div class="label">Next board<br>${nextSize[0]} × ${nextSize[1]}</div></div><div class="shopSection"><h3>MYSTERY DOMINO</h3><p>Random new physical tile. ${s.marketRandomStock} left. Every purchase raises all future prices.</p><button id="mysteryBuy" class="shopBuy">BUY RANDOM TILE · ${randomCost}c</button>${bought}</div><div class="shopSection"><h3>EXACT DOMINO</h3><p>Choose the exact values. Current price: ${exactCost} coins.</p>${exactGridHtml()}<div class="shopFoot">Purchased dominoes are new physical copies with unique IDs. Inflation persists for the whole run.</div></div></div>`;
    const mystery=$('mysteryBuy');mystery.disabled=s.coins<randomCost||s.marketRandomStock<=0;mystery.onclick=()=>{const r=GAME.buyMarketRandomTile();if(!r.ok){toast(r.reason==='coins'?'Not enough coins':'Sold out');return}GAME.save();toast(`[${r.tile.a}|${r.tile.b}] added · Inflation ${r.inflation}`);render()};
