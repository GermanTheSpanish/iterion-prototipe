(function(){
  ...
  function startTutorial(){
    activeRun=GAME;tutorial={step:0};GAME=window.IterionGame.createGame(E,{seed:3100,TARGETS:[1e9],STARTING_COINS:30});prepareTutorialHand('d2-2');H.bindRun(GAME.state().runId);localStorage.setItem('iterion.tutorialChoice.v1','made');handFx.fill('normal');showGame();updateTutorial()
  }
  ...
  function advanceTutorial(result){
    ...
    if(tutorial.step===4&&result.cleared){tutorial.step=5;tutorialInstruction.textContent=tutorialCopy[5];tutorialStep.textContent='LEARN MONOID · 6/6';tutorialPanel.hidden=false;resetOverlay();overlayTitle.textContent='SHOP & MARKET';overlayBody.innerHTML='<p><strong>Shop</strong> sells temporary supplies and extra dominoes during a round.</p><p><strong>Market</strong> appears between stages and offers lasting machine changes.</p>';overlayPrimary.textContent='FINISH';overlayPrimary.onclick=()=>{hideOverlay();leaveTutorial(true)};overlaySecondary.style.display='inline-block';overlaySecondary.textContent='TRY AGAIN';overlaySecondary.onclick=()=>{hideOverlay();startTutorial()}}
  }
  ...
  function tileModMarks(t){if(!t?.id)return'';const s=GAME.state(),marks=[];if(s.doubleDoubleTileId===t.id)marks.push(['DD','dd']);if(s.doubleEchoTileId===t.id)marks.push(['DE','de']);if(s.zeroMemoryTileId===t.id)marks.push(['ZM','zm']);return marks.map(([label,cls],i)=>`<i class="tileModMark ${cls}" style="--mod-shift:${marks.length===1?0:i?3:-3}px" aria-hidden="true"><span>${label[0]}</span><span>${label[1]}</span></i>`).join('')}
  ...
  function marketTileHtml(t,label=''){return`<span class="marketTile" aria-label="Domino ${t.a}|${t.b}${label?` · ${label}`:''}">${mini(t)}${label?`<small>${escapeHtml(label)}</small>`:''}</span>`}
  function renderMachineModStatus(s){const owned=s.mods.includes('long-run'),cap=D.ENDLESS_LONG_RUN_ACTIVATIONS||7,used=s.endlessLongRunActivations||0,remaining=Math.max(0,cap-used),visible=owned&&(!s.endlessMode||remaining>0);machineModStatusEl.hidden=!visible;if(!visible){machineModStatusEl.innerHTML='';return}const ratio=s.endlessMode?remaining/cap:1;machineModStatusEl.innerHTML=`<span>LONG CHAIN</span><i><b style="width:${ratio*100}%"></b></i>`;machineModStatusEl.setAttribute('aria-label',s.endlessMode?`Long Chain · ${remaining} of ${cap} Endless activations remaining`:'Long Chain ready')}
  ...
  $('learnMonoid').onclick=startTutorial;$('replayTutorial').onclick=startTutorial;$('startRun').onclick=()=>startNormal(false);continueRun.onclick=()=>startNormal(true);$('modeClassic').onclick=()=>storedState()?startNormal(true):startNormal(false);$('leaveTutorial').onclick=()=>leaveTutorial(false);$('gameSelectionButton').onclick=()=>{closeMenu();showSelection()};
