(function(){
  ...
  function exactGridHtml(){let html='<div class="exactGrid">';for(let a=0;a<=6;a++)for(let b=0;b<=6;b++){if(b<a)html+='<span class="exactTile blank"></span>';else html+=`<button class="exactTile" data-a="${a}" data-b="${b}">${a}|${b}</button>`}return html+'</div>'}
  function doubleDoubleGridHtml(){
    const s=GAME.state(),placed=new Set(s.placedTileIds||[]),groups=new Map();
    const doubles=s.set.filter(t=>t.a===t.b&&t.a>0).sort((a,b)=>a.a-b.a||a.id.localeCompare(b.id));
    return`<div class="doubleDoubleGrid">${doubles.map(t=>{const key=`${t.a}|${t.b}`,copy=(groups.get(key)||0)+1;groups.set(key,copy);const active=s.doubleDoubleTileId===t.id,status=placed.has(t.id)?'MACHINE':'AVAILABLE';return`<button class="doubleDoubleChoice${active?' active':''}" data-dd-id="${t.id}" ${active?'disabled':''}><strong>[${t.a}|${t.b}]${copy>1?` · ${copy}`:''}</strong><span>${active?'ACTIVE':status}</span></button>`}).join('')}</div>`
  }
  function showMarket(){
    resetOverlay();const s=GAME.state(),x=GAME.snapshot(),nextStage=x.stage.index+1,nextSize=D.BOARD_SIZES[Math.min(nextStage-1,D.BOARD_SIZES.length-1)],randomCost=GAME.marketRandomPrice(),exactCost=GAME.marketExactPrice(),doubleDoubleCost=GAME.marketDoubleDoublePrice();
    ...
    overlayBody.innerHTML=`<div class="bigShop"><div class="shopHero"><div><div class="label">Stage ${x.stage.index} complete</div><strong>${s.coins}c</strong><div class="shopInflation">Inflation ${s.inflation}</div><div class="shopSupply">SUPPLY ${supply} · ${nextMarket}</div></div><div class="label">Next board<br>${nextSize[0]} × ${nextSize[1]}</div></div><div class="shopSection"><h3>MYSTERY DOMINO</h3><p>Random new physical tile. No stock limit; coins and Inflation set the limit.</p><button id="mysteryBuy" class="shopBuy">BUY RANDOM TILE · ${randomCost}c</button>${bought}</div><div class="shopSection"><h3>DOUBLE DOUBLE</h3><p>Choose one physical double. Its first activation each Move is doubled: [3|3] ×9, [5|5] ×25, [4|4] +8. Later passes in the same Move use the normal operation. Buying another transfers Double Double. Active: ${active}.</p>${doubleDoubleGridHtml()}<div class="shopFoot">Current price: ${doubleDoubleCost}c. Double Double is permanent until transferred and each purchase raises Inflation.</div></div><div class="shopSection"><h3>EXACT DOMINO</h3><p>Choose the exact values. Current price: ${exactCost} coins.</p>${exactGridHtml()}<div class="shopFoot">Purchased dominoes are new physical copies with unique IDs. Inflation persists for the whole run.</div></div></div>`;
    ...
    overlayBody.querySelectorAll('.doubleDoubleChoice[data-dd-id]').forEach(b=>{if(!b.disabled)b.disabled=s.coins<doubleDoubleCost;b.onclick=()=>{const r=GAME.buyDoubleDouble(b.dataset.ddId);if(!r.ok){toast(r.reason==='coins'?'Not enough coins':r.reason==='active'?'Already active':'Unavailable');return}GAME.save();toast(`DOUBLE DOUBLE [${r.tile.a}|${r.tile.b}] · Inflation ${r.inflation}`);render()}});
    overlayBody.querySelectorAll('.exactTile[data-a]').forEach(b=>{b.disabled=s.coins<exactCost;b.title=`Buy [${b.dataset.a}|${b.dataset.b}] for ${exactCost} coins`;b.onclick=()=>{const r=GAME.buyMarketExactTile(Number(b.dataset.a),Number(b.dataset.b));if(!r.ok){toast('Not enough coins');return}GAME.save();toast(`[${r.tile.a}|${r.tile.b}] added · Inflation ${r.inflation}`);render()}});
