(function(){
  ...
  function showFailed(){
    ...
    const reason=noTiles?'The automatic POWER set could not be generated. Download the run file so this can be diagnosed.':limit?(stalled?'You used every move, but a stored or Shop Move can continue this round.':'You used every move for this round.'):s.failureReason==='no-legal-moves'?'No legal placements remain and there are no Rerolls left.':'No legal continuation remains.';
  ...
  async function endDrag(e){
    ...
    if(!tutorial&&generationBefore<(game.state().setGeneration||1))toast(`POWER SET ${game.state().setGeneration} · ×${game.snapshot().powerSets.powerMultiplier} UNLOCKED`);else if(!tutorial&&game.state().cleared)toast(`Round clear · ${fmt(game.state().score)}`);else if(!tutorial&&result.autoRerolls)toast(result.autoRerolls===1?`AUTO REROLL · ${(result.autoRerollSources||[])[0]==='free'?'FREE':'STORED'}`:`AUTO REROLL ×${result.autoRerolls}`);else if(!tutorial&&result.upgradeCoins)toast(`★ +${result.upgradeCoins} coins`)
  }

  function chooseCircuitTile(tileId){const r=GAME.chooseCircuitTile(tileId);if(!r.ok)return;press.cancel();persistGame();clearOutcomeDelay();render();const recovery=r.autoRerolls?(r.autoRerolls===1?` · AUTO REROLL · ${(r.autoRerollSources||[])[0]==='free'?'FREE':'STORED'}`:` · AUTO REROLL ×${r.autoRerolls}`):'';toast(`CIRCUIT RANK ${D.CIRCUIT_RANKS[r.after-1].roman}${recovery}`)}
